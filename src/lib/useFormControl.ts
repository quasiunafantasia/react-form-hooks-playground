...
import {
  createUseDebounceOnChangeStrategy,
  isDebounceStrategy
} from "./model-update/strategies/debounceOnChangeStrategy";
import {
  createUseOnBlur,
  isOnBlurStrategy
} from "./model-update/strategies/onBlurStrategy";
import {
  createUseOnChange,
  isOnchangeStrategy,
  ON_CHANGE_STRATEGY
} from "./model-update/strategies/onChangeStrategy";
import { UPDATE_STRATEGY } from "./model-update/strategies/strategy.type";
import {
  AsyncValidator,
  usePreValidator,
  Validator
} from "./model-update/usePreValidator";
...
export function useFormControl<T = any, Errors = string, AsyncErrors = string>(
  config: FormControlConfig<T, Errors, AsyncErrors> = {}
) {
  ...
  const debounceStrategyRunner = createUseDebounceOnChangeStrategy(
    config.debounce
  );
  const onBlurStrategyRunner = createUseOnBlur(isBlurred);
  const onChangeStrategyRunner = createUseOnChange();
  const { error, value, setValue, status } = usePreValidator(
    config.validator,
